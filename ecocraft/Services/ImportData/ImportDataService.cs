    ...
    ServerDataService serverDataService)
{
    ...
    public async Task ImportIconsConfig(Server server)
    {
        try
        {
            importSkillIcons(server,true);
            importCraftingtableIcons(server,true);
            importItemIcons(server);
            await dbContext.SaveChangesAsync();
        }
        catch (JsonException ex)
        {
            Console.WriteLine($"Error during JSON icons deserialization: {ex.Message}");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error during icons import: {ex.Message} {ex.StackTrace}");
        }
    }

    private void importSkillIcons(Server server, bool debug = false)
    {
        var options = new JsonSerializerOptions();
        options.Converters.Add(new LanguageCodeDictionaryConverter());

        string jsonPath = Path.Combine(AppContext.BaseDirectory, "Ressources", "skills_icons.json");
        string jsonString = File.ReadAllText(jsonPath);

        var importedData = JsonSerializer.Deserialize<List<IconConfigDto>>(jsonString, options);

        if (importedData is not null)
        {
            foreach (var icon in importedData)
            {
                if (debug) Console.WriteLine($"NameID: {icon.nameID}, ImageFile: {icon.imageFile}, X: {icon.xPos}, Y: {icon.yPos}");
                Skill? skill = server.Skills.FirstOrDefault(s => s.Name == icon.nameID);
                if (skill != null)
                {
                    skill.imageFile = icon.imageFile;
                    skill.posX = icon.xPos;
                    skill.posY = icon.yPos;
                }
                else
                {
                    if (debug) Console.WriteLine($"\tERROR during SKILL icons mapping: NameID {icon.nameID} not found");
                }
            }            
        }
    }

    private void importCraftingtableIcons(Server server, bool debug = false)
    {
        var options = new JsonSerializerOptions();
        options.Converters.Add(new LanguageCodeDictionaryConverter());

        string jsonPath = Path.Combine(AppContext.BaseDirectory, "Ressources", "crafting_icons.json");
        string jsonString = File.ReadAllText(jsonPath);
        ...
        if (importedData is not null)
        {
            ...
            {
                if(debug) Console.WriteLine($"NameID: {icon.nameID}, ImageFile: {icon.imageFile}, X: {icon.xPos}, Y: {icon.yPos}");
                CraftingTable? craftinTable = server.CraftingTables.FirstOrDefault(s => s.Name == icon.nameID);
                if (craftinTable != null)
                {
                    craftinTable.imageFile = icon.imageFile;
                    craftinTable.posX = icon.xPos;
                    craftinTable.posY = icon.yPos;
                }
                else
                {
                    if (debug) Console.WriteLine($"\tERROR during TABLE icons mapping: NameID {icon.nameID} not found");
                }
    ...
    private void importItemIcons(Server server, bool debug = false)
    {
        var options = new JsonSerializerOptions();
        options.Converters.Add(new LanguageCodeDictionaryConverter());

        string jsonPath = Path.Combine(AppContext.BaseDirectory, "Ressources", "items_icons.json");
        string jsonString = File.ReadAllText(jsonPath);
        ...
        if (importedData is not null)
        {
            ...
            {
                if (debug) Console.WriteLine($"NameID: {icon.nameID}, ImageFile: {icon.imageFile}, X: {icon.xPos}, Y: {icon.yPos}");
                ItemOrTag? itemOrTag = server.ItemOrTags.FirstOrDefault(s => s.Name == icon.nameID);
                if (itemOrTag != null)
                {
                    itemOrTag.imageFile = icon.imageFile;
                    itemOrTag.posX = icon.xPos;
                    itemOrTag.posY = icon.yPos;
                }
                else
                {
                    if (debug) Console.WriteLine($"\tERROR during ITEMS icons mapping: NameID {icon.nameID} not found");
                }                
            }
        }
    }
